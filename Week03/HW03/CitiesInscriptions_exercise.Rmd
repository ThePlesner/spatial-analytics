...
author: "Adela Sobotkova, exercise made by Mikkel Plesner Ottosen"
date: "21/02/2021 updated `r format(Sys.time(), '%B %d, %Y')`" 
output:
...
```
We can verify that is it projected by seeing the projection attribute and that the crs is EPSG:3035

### Question 1: 
*What are the measurement units of the `cities_sf3035` object?*
As per default, the measurement unit will be in meters and looking at the data we can see that the unit is meter in most if not all of the entries. 
...
tm_shape(ten_buffers)  +
  ...
  tm_shape(ten_cities) +
  tm_text("Ancient Toponym", size = 0.4, auto.placement = 5) +
  tm_dots(col = "hotpink", 
             size = 0.1) +
...
#I use regex to "clean" the coordinates attribute before its separated using gsub. The regex finds everything that is not numbers 0-9, comma, and period, and replaces it with nothing. 
i_sm <- inscriptions %>% 
  slice(1:100) %>% 
  mutate(coordinates = gsub("[^0-9.,]+", "", coordinates)) %>%
...
We can see that they are from all from Italy. 
...
```{r insc-sf, eval=TRUE}
#using the experimentation from last exercise i insert the same regex and subsitutes the mathces with nothing. 
#In the first filter i ask the function to only let a certain row pass, if neither the longitude or latitude is NA. 
i <- inscriptions %>% 
  mutate(coordinates = gsub("[^0-9.,]+", "", coordinates)) %>%
  separate(col = coordinates, into = c("longitude","latitude"), sep = ',') %>%
  mutate(latitude = as.numeric(latitude),
  ...
  filter(latitude > 5 & longitude < 20) %>% 
...
The last 3 lines in the code prints the number of rows in the two variables aswell as the ratio. Looking at the ratio, we can see that there is a much higher amount of inscriptions in urban areas rather than rural. 